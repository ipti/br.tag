...
select
	...
	(select
		group_concat(
			(case `fa`.`allergy_type`
				when 'LEITE' then 'Leite'
				when 'OVO' then 'Ovo'
				when 'PEIXE' then 'Peixe'
				when 'CRUSTACEOS_MOLUSCOS' then 'Crustáceos e molusco'
				when 'AMENDOIM' then 'Amendoim'
				when 'ABACAXI' then 'Abacaxi'
				when 'CASTANHAS_OLEAGINOSAS' then 'Castanhas e outras oleaginosas'
				when 'SOJA' then 'Soja'
				when 'TRIGO' then 'Trigo'
				when 'GERGELIM' then 'Gergelim'
				when 'OUTRAS' then coalesce(`fa`.`description`, 'Outras')
				else `fa`.`allergy_type`
			end)
			separator ', ')
	from `student_food_allergy` `fa`
	where `fa`.`student_fk` = `s`.`id`) as `food_allergies_detail`,
