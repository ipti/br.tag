...
class ClassBoardController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('index','view','getClassBoard','create','update', 'addLesson','updateLesson','deleteLesson'),
				'users'=>array('@'),
			),
	...
	public function actionView($id)
	{
        ...
        public function actionDeleteLesson($lesson = null){
            ...
            $schedule_final = ($final_timestamp == -1) 
                    ? ($schedule_initial+1)
                    : date('G',$final_timestamp);
            ...
            switch ($day){
                case 0: $classboard->week_day_sunday    = $schedule; break;
                case 1: $classboard->week_day_monday    = $schedule; break;
                case 2: $classboard->week_day_tuesday   = $schedule; break;
                case 3: $classboard->week_day_wednesday = $schedule; break;
                case 4: $classboard->week_day_thursday  = $schedule; break;
                case 5: $classboard->week_day_friday    = $schedule; break;
                case 6: $classboard->week_day_saturday  = $schedule; break;
            }
        ...
        public function actionAddLesson($lesson = null){
            ...
            if ($classboard == null) {
                ...
                switch ($week_day){
                    case 0: $schedule = $classboard->week_day_sunday;   break;
                    case 1: $schedule = $classboard->week_day_monday;   break;
                    case 2: $schedule = $classboard->week_day_tuesday;  break;
                    case 3: $schedule = $classboard->week_day_wednesday;break;
                    case 4: $schedule = $classboard->week_day_thursday; break;
                    case 5: $schedule = $classboard->week_day_friday;   break;
                    case 6: $schedule = $classboard->week_day_saturday; break;
                }
                $schedule = $schedule == '0' ? array() : explode(';', $schedule);
            }
            
            for ($i = $schedule_initial; $i < $schedule_final; $i++) { array_push($schedule, $i); }
            ...
            if ($classboard->validate() && $classboard->save()) {
                ...
                $event = array(
                    'id' => $lesson['id'],
                    'db' => $classboard->id,
                    'title' => strlen($lesson['title']) > 40 
                                ? substr($lesson['title'],0,37) . "..."
                                : $lesson['title'],
                    'discipline' => $lesson['discipline'],
                    'classroom' => $lesson['classroom'],
                    'start' => $lesson['start'],
        ...
        public function actionGetClassBoard(){
            ...
            if(!isset($_POST['ClassBoard']['classroom_fk']) || empty($_POST['ClassBoard']['classroom_fk'])) return null;
                    ...
                    foreach($d as $schedule){
                        if($schedule != 0){
                            $event = array(
                                'id' => ++$lessons,
                                'db' => $cb->id,
                                'title' => strlen($discipline->name) > 40 
                                            ? substr($discipline->name,0,37) . "..."
                                            : $discipline->name,
