...
//@done S2 - Modularizar o cÃƒÆ’Ã‚Â³digo do import
//@done S2 - Criar o controller de Import
//@done S2 - Mover o cÃƒÆ’Ã‚Â³digo do import de SchoolController.php para AdminController.php
//@done S2 - Mover o cÃƒÆ’Ã‚Â³digo do configACL de SchoolController.php para AdminController.php
//@done S2 - Criar mÃƒÆ’Ã‚Â©todo de limparBanco
//@done S2 - Criar tela de index do AdminController.php
//@done S2 - Criar usuÃƒÆ’Ã‚Â¡rios padrÃƒÆ’Ã‚Âµes.
...
class CensoController extends Controller
{
        ...
        return [
            ...
            [
                ...
                'actions' => [
                    ...
                    'importFromMaster',
                    'downloadexportfile',
                    'downloadexportfileidentification',
                    'ExportWithoutInepid'
                ],
                'users' => ['@'],
            ],
    ...
    public function validateSchool($schoolIdentificationColumn, $managerIdentificationColumn)
    {
        ...
        if (!$result['status']) {
            array_push($log, ['register_type' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['situation' => $result['erro']]);
        }
        ...
        if ($schoolIdentificationColumn['address_number'] !== '' && $schoolIdentificationColumn['address_number'] !== null) {
            ...
            if (!$result['status']) {
                array_push($log, ['address_number' => $result['erro']]);
            }
        }
        ...
        if ($schoolIdentificationColumn['address_neighborhood'] !== '' && $schoolIdentificationColumn['address_neighborhood'] !== null) {
            ...
            if (!$result['status']) {
                array_push($log, ['address_neighborhood' => $result['erro']]);
            }
        }
        ...
        if (!$result['status']) {
            array_push($log, ['DDD e Telefones' => $result['erro']]);
        }
        ...
        if ($schoolIdentificationColumn['email'] !== '' && $schoolIdentificationColumn['email'] !== null) {
            ...
            if (!$result['status']) {
                array_push($log, ['email' => $result['erro']]);
            }
        }
        ...
        if (!$result['status']) {
            array_push($log, ['administrative_dependence' => $result['erro']]);
        }

        //campo 29
        $result = $siv->isLocationValid($schoolIdentificationColumn['location']);
        if (!$result['status']) {
            array_push($log, ['location' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['private_school_category' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['public_contract' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['private_school_maintainer_cnpj' => $result['erro']]);
        }
        ...
        if (!empty($check)) {
            ...
            if (!$result['status']) {
                array_push($log, ['inep_head_school' => $result['erro']]);
            }
        }
        //campo 42
        ...
        if (!$result['status']) {
            array_push($log, ['ies_code' => $result['erro']]);
        }

        $result = $siv->isNotNullValid($schoolIdentificationColumn['id_difflocation']);
        if (!$result['status']) {
            array_push($log, ['id_difflocation' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Orgao ao qual a escola publica esta vinculada' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Esfera do Orgao Regulador' => $result['erro']]);
        }

        //campo 3 - ok
        $result = $siv->isManagerCPFValid($managerIdentificationColumn['cpf']);
        if (!$result['status']) {
            array_push($log, ['CPF do Gestor Escolar' => $result['erro']]);
        }

        //campo 4 - ok
        $result = $siv->isManagerNameValid($managerIdentificationColumn['name']);
        if (!$result['status']) {
            array_push($log, ['Nome do Gestor Escolar' => $result['erro']]);
        }

        $year = Yii::app()->user->year;
        $result = $siv->isManagerBirthdayValid($managerIdentificationColumn['birthday_date'], $year);
        if (!$result['status']) {
            array_push($log, ['Data de Nascimento do Gestor Escolar' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Cidade do Gestor Escolar' => $result['erro']]);
        }

        $result = $siv->isAreaOfResidenceValid($managerIdentificationColumn['residence_zone']);
        if (!$result['status']) {
            array_push($log, ['Localizacao/Zona de residencia do Gestor Escolar' => $result['erro']]);
        }

        //campo 6 - ok
        $result = $siv->isManagerEmailValid($managerIdentificationColumn['email']);
        if (!$result['status']) {
            array_push($log, ['E-mail do Gestor Escolar' => $result['erro']]);
        }
    ...
    public function validateSchoolStructure($collumn, $school)
    {
        ...
        if (!$result['status']) {
            array_push($log, ['building_occupation_situation' => $result['erro']]);
        }

        //campos 14 ÃƒÆ’Ã‚Â  19
        $sharedSchoolInepIds = [
        ...
        if (!$result['status']) {
            array_push($log, ['Esgoto' => $result['erro']]);
        }

        //campos 33 ÃƒÆ’Ã‚Â  38
        $garbageDestinations = [
        ...
        if (!$result['status']) {
            array_push($log, ['Tratamento do lixo' => $result['erro']]);
        }

        //campos 39 ÃƒÆ’Ã‚Â  68
        $dependencies = [
        ...
        if (!$result['status']) {
            array_push($log, ['Dependencias' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Acessibilidade' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Acesso ÃƒÆ’Ã‚Â  Internet' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Instrumentos, materiais socioculturais e/ou pedagogicos em uso na escola para o desenvolvimento de atividades de ensino aprendizagem' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Orgaos em Funcionamento na Escola' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['feeding' => $result['erro']]);
        }
    ...
    public function validateClassroom($column, $school)
    {
        ...
        if (!$result['status']) {
            array_push($log, ['school_inep_fk' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['pedagogical_mediation_type' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['classroom_time' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['classroom_days' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Atividades Complementares' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Professores' => $result['erro']]);
        }
    ...
    public function validateInstructor($collumn, $instructorDocumentsAndAddress)
    {
        ...
        if (!$result['status']) {
            array_push($log, ['birthday_date' => $result['erro']]);
        }

        //campo 9
        $result = $iiv->oneOfTheValues($collumn['sex']);
        if (!$result['status']) {
            array_push($log, ['sex' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['nationality' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['deficiency' => $result['erro']]);
        }
        ...
        if (!isset($variabelData)) {
            array_push($log, ['Escolaridade' => 'FormaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o educacional nÃƒÆ’Ã‚Â£o foi informada']);
        }
    ...
    public function validateInstructorDocuments($collumn)
    {
        ...
        if (!empty($instructorInepId)) {
            ...
            $check = Yii::app()->db->createCommand($sql)->queryAll();
            $result = $idav->isEqual($check[0]['status'], '1', "NÃƒÆ’Ã‚Â£o hÃƒÆ’Ã‚Â¡ tal inep_id $instructorInepId");
            if (!$result['status']) {
                array_push($log, ['id' => $result['erro']]);
            }
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Localizacao/Zona de residencia' => $result['erro']]);
        }
    ...
    public function validateInstructorData($collumn)
    {
        ...
        $result = $itdv->isEqual($check[0]['status'], '1', 'NÃƒÆ’Ã‚Â£o hÃƒÆ’Ã‚Â¡ tal instructor_fk $instructorFk');
        if (!$result['status']) {
        ...
        if (!$result['status']) {
            array_push($log, ['classroom_inep_id' => $result['erro']]);
        }
        ...
        $result = $itdv->isEqual($check[0]['status'], '1', 'NÃƒÆ’Ã‚Â£o hÃƒÆ’Ã‚Â¡ tal classroom_id_fk $classroomFk');
        if (!$result['status']) {
        ...
        if (!$result['status']) {
            array_push($log, ['role' => $result['erro']]);
        }
    ...
    public function validateStudentIdentification($collumn, $studentDocument, $classroom)
    {
        ...
        //campo 17 ÃƒÆ’Ã‚Â  24 e 26 ÃƒÆ’Ã‚Â  29
        ...
        $excludingdeficiencies = [
            ...
            ['Baixa VisÃƒÆ’Ã‚Â£o' => [$collumn['deficiency_type_low_vision'] => [$collumn['deficiency_type_deafblindness']]]],
            ['Surdez' => [$collumn['deficiency_type_deafness'] => [$collumn['deficiency_type_disability_hearing'], $collumn['deficiency_type_deafblindness']]]],
            ['DeficiÃƒÆ’Ã‚Âªncia Auditiva' => [$collumn['deficiency_type_disability_hearing'] => [$collumn['deficiency_type_deafblindness']]]]
        ];
        ...
        if ($classroom['aee'] == 1 && ($collumn['deficiency'] == 0 || !$result['status'])) {
            array_push($log, ['Deficiencia' => 'O aluno estÃƒÆ’Ã‚Â¡ matriculado na turma de Atendimento Educacional Especializado "' . $classroom['name'] . '", mas nenhum tipo de deficiÃƒÆ’Ã‚Âªncia foi marcado.']);
        }
    ...
    public function validateStudentDocumentsAddress($collumn, $studentident)
    {
        ...
        if (!empty($studentInepId)) {
            ...
            if ($count === 0) {
                array_push($log, ['student_indentification' => "INEP do aluno nÃƒÆ’Ã‚Â£o encontrado: $studentInepId"]);
            } elseif ($count > 1) {
        ...
        if (!$result['status']) {
            array_push($log, ['foreign_document_or_passport' => $result['erro']]);
        }
    ...
    public function validateEnrollment($collumn)
    {
        ...
        if (!empty($studentInepIdFk)) {
            ...
            $count = (int) $check[0]['status'];
            if ($count === 0) {
                array_push($log, ['student_fk' => "INEP do aluno nÃƒÆ’Ã‚Â£o encontrado: $studentInepIdFk"]);
            } elseif ($count > 1) {
        ...
        $result = $sev->isEqual($check[0]['status'], '1', 'NÃƒÆ’Ã‚Â£o hÃƒÆ’Ã‚Â¡ tal classroom_id $classroomFk');
        if (!$result['status']) {
        ...
        if (!$result['status']) {
            array_push($log, ['enrollment_id' => $result['erro']]);
        }
        ...
        if (!$result['status']) {
            array_push($log, ['Tipo de Atendimento Educacional Especializado' => $result['erro']]);
        }
    ...
    public function fixName($name)
    {
        ...
        $name = str_replace(['ÃƒÆ’Ã‚Â', 'ÃƒÆ’Ã¢â€šÂ¬', 'ÃƒÆ’Ã†â€™', 'ÃƒÆ’Ã¢â‚¬Å¡', 'ÃƒÆ’Ã¢â‚¬Å¾'], 'A', $name);
        $name = str_replace(['ÃƒÆ’Ã¢â‚¬Â°', 'ÃƒÆ’Ã‹â€ ', 'ÃƒÆ’Ã…Â ', 'ÃƒÆ’Ã¢â‚¬Â¹'], 'E', $name);
        $name = str_replace(['ÃƒÆ’Ã‚Â', 'ÃƒÆ’Ã…â€™', 'ÃƒÆ’Ã…Â½', 'ÃƒÆ’Ã‚Â'], 'I', $name);
        $name = str_replace(['ÃƒÆ’Ã¢â‚¬Å“', 'ÃƒÆ’Ã¢â‚¬â„¢', 'ÃƒÆ’Ã¢â‚¬Â¢', 'ÃƒÆ’Ã¢â‚¬Â', 'ÃƒÆ’Ã¢â‚¬â€œ'], 'O', $name);
        $name = str_replace(['ÃƒÆ’Ã…Â¡', 'ÃƒÆ’Ã¢â€žÂ¢', 'ÃƒÆ’Ã¢â‚¬Âº', 'ÃƒÆ’Ã…â€œ'], 'U', $name);
        $name = str_replace(['ÃƒÆ’Ã¢â‚¬Â¡'], 'C', $name);
        return $name;
    }
    ...
    {
        $what = ['ÃƒÆ’Ã‚Â¤', 'ÃƒÆ’Ã‚Â£', 'ÃƒÆ’Ã‚Â ', 'ÃƒÆ’Ã‚Â¡', 'ÃƒÆ’Ã‚Â¢', 'ÃƒÆ’Ã‚Âª', 'ÃƒÆ’Ã‚Â«', 'ÃƒÆ’Ã‚Â¨', 'ÃƒÆ’Ã‚Â©', 'ÃƒÆ’Ã‚Â¯', 'ÃƒÆ’Ã‚Â¬', 'ÃƒÆ’Ã‚Â­', 'ÃƒÆ’Ã‚Â¶', 'ÃƒÆ’Ã‚Âµ', 'ÃƒÆ’Ã‚Â²', 'ÃƒÆ’Ã‚Â³', 'ÃƒÆ’Ã‚Â´', 'ÃƒÆ’Ã‚Â¼', 'ÃƒÆ’Ã‚Â¹', 'ÃƒÆ’Ã‚Âº', 'ÃƒÆ’Ã‚Â»', 'ÃƒÆ’Ã¢â€šÂ¬', 'ÃƒÆ’Ã‚Â', 'ÃƒÆ’Ã¢â‚¬Â°', 'ÃƒÆ’Ã‚Â', 'ÃƒÆ’Ã¢â‚¬Å“', 'ÃƒÆ’Ã…Â¡', 'ÃƒÆ’Ã‚Â±', 'ÃƒÆ’Ã¢â‚¬Ëœ', 'ÃƒÆ’Ã‚Â§', 'ÃƒÆ’Ã¢â‚¬Â¡', ' ', '-', '(', ')', ',', ';', ':', '|', '!', '"', '#', '$', '%', '&', '/', '=', '?', '~', '^', '>', '<', 'Ãƒâ€šÃ‚Âª', 'Ãƒâ€šÃ‚Âº'];
    ...
    public function fixMistakesExport($register, $attributes)
    {
        switch ($register) {
            ...
            case '70':
                ...
                } elseif ($attributes['civil_certification'] == 2) {
            ...
            case '51':
                /*
                 * O campo "CÃƒÆ’Ã‚Â³digo da disciplina 4" nÃƒÆ’Ã‚Â£o pode ser preenchido quando a turma ÃƒÆ’Ã‚Â  qual o docente
                 * estÃƒÆ’Ã‚Â¡ vinculado for de EducaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o infantil ou EJA - Fundamental - Projovem urbano.
                 */
    ...
    public function actionExport($withoutCertificates)
    {
        ...
        Yii::app()->user->setFlash('error', Yii::t('default', 'Houve algum erro na ExportaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o.'));
        return $this->redirect(['index']);
    }

    public function actionExportIdentification($withoutCertificates = false)
    {
        include_once dirname(__DIR__) . '/libraries/Educacenso/Educacenso.php';
        $educacenso = new Educacenso();
        $export = $educacenso->exportarIdentification((bool) $withoutCertificates);
        ...
        if ($result) {
            $school = SchoolIdentification::model()->findByPk(Yii::app()->user->school);
            Log::model()->saveAction('educacenso', Yii::app()->user->school, 'E', $school->name);
            return $this->redirect(['index', 'exportedIdentification' => 1]);
        }
    ...
    public function actionDownloadExportFile()
    {
        ...
        if (file_exists($fileDir)) {
            ...
            Yii::app()->user->setFlash('error', Yii::t('default', 'Arquivo de exportaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o nÃƒÆ’Ã‚Â£o encontrado!!! Tente exportar novamente.'));
            $this->render('index');
        }
    }
    ...
    public function fileImportCorrectIneps($uploadedFile)
    {
        try {
            ...
            if ($file === false) {
                die('O arquivo nÃƒÆ’Ã‚Â£o existe.');
            }
            while (true) {
                $fileLine = fgets($file);
                if ($fileLine == null) {
            ...
            Yii::app()->user->setFlash('success', 'ImportaÃƒÆ’Ã‚Â§ÃƒÆ’Ã‚Â£o realizada com sucesso!');
