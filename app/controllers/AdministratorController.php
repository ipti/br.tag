    ...
    class AdministratorController extends Controller
    {
        ...
        public function actionSyncImport()
        {
            ...
            foreach ($students as $student) {
                ...
                $studentValues[] = $student['attributes'];
//            echo $student['attributes']['id']. '-' .$student['attributes']['fkid'].'<br>';
            ...
            foreach ($students as $student) {
                ...
                $documentValues[] = $document['attributes'];
            }
            ...
            foreach ($classrooms as $classroom) {
                ...
                $classroomValues[] = $classroom['attributes'];
            }
            ...
            foreach ($classrooms as $classroom) {
                foreach ($classroom['classes'] as $class) {
                    ...
                    $classesValues[] = $class['attributes'];
                }
            }
            ...
            foreach ($classrooms as $classroom) {
                foreach ($classroom['enrollments'] as $enrollment) {
                    ...
                    $enrollmentsValues[] = $enrollment['attributes'];
                }
            }
        ...
        public function actionUpdateDB()
        {
            ...
            foreach ($dirFiles as $fileName) {
                if ('.' != $fileName && '..' != $fileName && 'readme' != $fileName && '_version' != $fileName && '~' != substr('abcdef', -1)) {
...
//        Solução paliativa para evitar explosão
        ...
        private function getInsertValues($registerLines)
        {
            foreach ($registerLines as $regType => $lines) {
                ...
                $isSchoolIdentification = ('00' == $regType);
                $isSchoolStructure = ('10' == $regType);

                $isRegInstructorIdentification = ('30' == $regType);
                $isRegInstructorDocumentsAndAddress = ('40' == $regType);
                $isRegInstructorVariableData = ('50' == $regType);
                $isRegInstructorTeachingData = ('51' == $regType);

                $isStudentIdentification = ('60' == $regType);
                $isStudentDocumentsAndAddress = ('70' == $regType);
                $isStudentEnrollment = ('80' == $regType);
                ...
                for ($line = 0; $line <= $totalLines; ++$line) {
                    ...
                    for ($column = 0; $column <= $totalColumns; ++$column) {
                        ...
                        if ($isRegInstructorTeachingData && 19 == $totalColumns) {
                            $lines[$line][count($lines[$line]) - 1] = 'null';
                            ++$totalColumns;
                        }

                        if (($isSchoolStructure && 105 == $totalColumns) || ($isRegInstructorIdentification && 22 == $totalColumns) || ($isRegInstructorDocumentsAndAddress && 11 == $totalColumns) || ($isStudentIdentification && 37 == $totalColumns) || ($isStudentEnrollment && 22 == $totalColumns) || ($isStudentDocumentsAndAddress && 27 == $totalColumns)) {
                            $lines[$line][count($lines[$line])] = 'null';
                            ++$totalColumns;
                        }

                        $value = ('null' == $value || $withoutcomma) ? $value : "'$value'";
        ...
        public function actionExportToMaster()
        {
            ...
            for ($i = 0; $i < count($tables); ++$i) {
                ...
                foreach ($objects as $object) {
                    if (0 == $i) { // remover atributo blob do school_identification
        ...
        public function actionCleanMaster()
        {
            ...
            foreach ($instructorTgs as $row) {
                ...
                $conflicts['instructors'][] = [$instructor1, $instructor2];
            }
            ...
            foreach ($studentTgs as $row) {
                ...
                $conflicts['students'][] = [$student1, $student2];
            }
