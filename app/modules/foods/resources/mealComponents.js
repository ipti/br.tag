...
const MenuComponent = function () {
  ...
  function render(){
    meals.forEach((meal) => MealsComponent(meal).render());
  }
...
const MealsComponent = function ({day, hour, type, turn, plates}) {
  ...
  function render(idAccordion) {
    ...
    wrapper.find('.js-add-plate').on('click', function() {
      addPlate({name: dsadsadsa})
    });
    container.append(wrapper.children())
  
    plates.forEach((plate)=> PlatesComponent(plate).render());
...
const PlatesComponent = function () {
    ...
    $('js-add-plate').on('addPlateEvent', function(event, idAccordion) {
