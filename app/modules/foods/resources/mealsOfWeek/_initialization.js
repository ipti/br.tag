...
$(document).on(
    "change",
    ".js-filter-turns, .js-filter-public-target",
    function (e) {
        ...
        days.forEach((day) => {
            if (turnFilter.length > 0) {
                mealsOfWeekFiltered[day] = mealsOfWeekFiltered[day].filter(
                    (item) => {
                ...
                mealsOfWeekFiltered[day] = mealsOfWeekFiltered[day].filter(
                    (item) => {
                        return item.stages.some((stage) =>
                            publicTargetFilter.includes(String(stage))
                        );
                    }
                );
            }
...
function createCard(meal_component, meal, dayMeal) {
    ...
    return `<div class="t-cards ${
        dayMeal != day ? "hide" : ""
    }"  style=" max-width: none;" data-public-target="${
        meal.foodPublicTargetId
    }" data-turn="${turn}">
                <div class="t-cards-content">
                    ...
                    <div class="t-cards-text clear-margin--left">Ingredientes: ${igredients.join(
                        ", "
                    )} </div>
                </div>
            </div>`;
