...
function getFoodInventory() {
    $.ajax({
        ...
        renderStockTotals(food_inventory);
...
$(document).on("click", "#js-status-button", function () {
    ...
    $("#js-saveFoodInventoryStatus").attr(
        "data-foodInventoryId",
        foodInventoryId
    );
...
$(document).on("click", "#add-food", function () {
    ...
    if (foodId == "alimento" || amount == "") {
        $("#stock-modal-alert")
            ...
            .html("Campos obrigatórios precisam ser informados.");
    } else {
        if (
            amount !== "" &&
            !isNaN(amount) &&
            ...
            $("#stock-modal-alert")
                ...
                .addClass("alert-error")
                .html(
                    "Quantidade informada não é válida, utilize números positivos e se decimal, separe por '.'"
                );
        }
    }
});
        ...
        $.ajax({
            ...
            .success(function (response) {
                ...
                $("#info-alert")
                    ...
                    .html("Alimento(s) adicionado(s) ao estoque com sucesso.");
                getFoodInventory();
            })
            .fail(function (error) {
                $("#info-alert")
                    .removeClass("hide")
                    .addClass("alert-error")
                    .html("Não foi possível adicionar o alimento no sistema.");
            });
    } else {
        $("#stock-modal-alert")
            .removeClass("hide")
            .addClass("alert-error")
            .html(
                "Para adicionar ao estoque é necessário adicionar um alimento"
            );
    }
});
        ...
        $.ajax({
            ...
            .success(function (response) {
                ...
                $("#info-alert")
                    ...
                    .addClass("alert-success")
                    .html(
                        "Estoque de alimento retirado do sistema com sucesso."
                    );
            })
            .fail(function (error) {
                ...
                $("#info-alert")
                    ...
                    .addClass("alert-error")
                    .html(
                        "Não foi possível retirar o estoque do alimento do sistema."
                    );
            });
    }
});
        ...
        $.ajax({
            ...
            .success(function (response) {
                ...
                $("#info-alert")
                    .removeClass("hide")
                    .addClass("alert-success")
                    .html("Status do alimento modificado com sucesso.");
            })
            .fail(function (error) {
                $("#info-alert")
                    .removeClass("hide")
                    .addClass("alert-error")
                    .html("Não foi possível modificar o status do alimento.");
            });
    }
});
...
$(document).on("change", "#food", function () {
    ...
    switch (measurementUnit) {
        case "g":
            measurementUnitSelect.append(
                $(
                    '<option value="unidade" selected>Unidade</option><option value="g">g</option><option value="Kg">Kg</option>'
                )
            );
            break;
        case "u":
