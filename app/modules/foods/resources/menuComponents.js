...
const PlateComponent = function (plate) {
  ...
  function calculateNutritionalValue(table) {
    let total_pt = total_lip = total_car = total_kcal = 0;
    ...
    table.find('.js-car').each((_, car) => {
      total_car+= Number(car.innerHTML) ? Number(car.innerHTML) : 0
    })
    ...
    const lineTotal = $(`<tr class='js-total'></tr>`)
      ...
      .append(`<td>${total_car.toFixed(2)}</td>`)
      .append(`<td>${total_kcal.toFixed(2)}</td>`)
      .append(`<td></td>`)
    table.append(lineTotal)
  }
  function createMealComponent({ id, name, pt, lip, car, kcal }, food) {
