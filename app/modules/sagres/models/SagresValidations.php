...
class SagresValidations
{
    ...
    public function validatorAttendance($professional)
    {
        ...
        foreach ($attendances as $attendance) {
            $dateOfAttendance = (int) $attendance->getData()->format('Y');
            $currentDate = date('Y');
    ...
    public function validatorSchoolDirector($school)
    {
        ...
        if (null == $school->getDiretor()->getNrAto()) {
            $inconsistencies[] = [
                'enrollment' => 'DIRETOR',
        ...
        if (null == $school->getDiretor()->getCpfDiretor() || !preg_match('/^[0-9]{11}$/', $school->getDiretor()->getCpfDiretor())) {
            $inconsistencies[] = [
                'enrollment' => 'DIRETOR',
                'school' => $school->getIdEscola(),
    ...
    public function validatorClass($school, $finalClass)
    {
        ...
        foreach ($classes as $class) {
            ...
            if (strlen($class->getDescricao()) <= $strlen && null !== $class->getDescricao()) {
                $inconsistencies[] = [
                    'enrollment' => 'TURMA',
                    'school' => $school->getIdEscola(),
    ...
    public function validationStudent($student, $schoolId, $class)
    {
        ...
        if (!is_bool((bool) $student->getPcd())) {
            $inconsistencies[] = [
                'enrollment' => 'ESTUDANTE',
                'school' => $schoolId,
