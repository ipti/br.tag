...
class DefaultController extends Controller
{
    ...
    public function actionUpdateQuiz($id)
    {
        ...
        $this->render(self::RENDER_QUIZ_UPDATE, ['quiz' => $quiz, 'quizQuestion' => $quizQuestion]);
    }
    ...
    public function actionUpdateGroup($id)
    {
        ...
        $this->render(self::RENDER_GROUP_UPDATE, ['group' => $group]);
    }
        ...
        if (isset($_POST['QuestionGroup'])) {
            if (count($questions) == 0) {
                if ($group->delete()) {
                    ...
                    return $this->render(self::RENDER_GROUP_UPDATE, ['group' => $group]);
                }
            } else {
                $group->attributes = $_POST['QuestionGroup'];
    ...
    public function actionCreateQuestionGroup()
    {
        ...
        if (isset($_POST['QuestionGroupQuestion'])) {
            ...
            $existsQuestionGroup = QuestionGroupQuestion::model()->findByPk(
                [
                    'question_group_id' => $_POST['QuestionGroupQuestion']['question_group_id'],
                    'question_id' => $_POST['QuestionGroupQuestion']['question_id']
                ]
            );

            if (is_null($existsQuestionGroup)) {
                if ($questionGroup->validate() && $questionGroup->save()) {
    ...
    {
        $questionGroup = QuestionGroupQuestion::model()->findByPk(
            [
            ...
            if ($questionGroup->validate()) {
                if ($questionGroup->updateByPk(
                    ...
                    [
                        ...
                        'question_id' => $questionGroup->question_id
                    ]
                )) {
                    Yii::app()->user->setFlash('success', Yii::t(
                        'default',
                        'Grupo de questões atualizado com sucesso'
                    ));
                } else {
    ...
    public function actionAnswer($quizId, $studentId)
    {
        if (isset($_POST['FormQuestion'])) {
            ...
            $answered = Answer::model()->find(
                'quiz_id=:quiz_id AND student_id=:student_id',
                [
                    ':quiz_id' => $quizId,
