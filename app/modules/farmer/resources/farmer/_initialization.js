...
$(document).ready(function() {
    ...
    if($id != null) {
        $.ajax({
            type: 'POST',
            url: "?r=farmer/default/getFarmerFoods",
            cache: false,
            data: {
                id: $id,
            }
            ...
            $.ajax({
                type: 'POST',
                url: "?r=farmer/default/getFarmerDeliveries",
                cache: false,
                data: {
                    farmer: $id,
                }
...
$(document).on("focusout", "#farmerCpf", function () {
    ...
    if(id == null && farmerCpf != '') {
        if(farmerCpf.length < 11) {
            ...
            $.ajax({
                type: 'POST',
                url: "?r=farmer/default/getFarmerRegister",
                cache: false,
                data: {
                    farmerCpf: farmerCpf,
                }
                ...
                if("error" in farmerRegister) {
                    if(farmerRegister.error == "Existente ativo") {
                        ...
                        $('#info-alert').removeClass('hide').addClass('alert-error').html(
                            ...
                            Para ativar o agricultor, clique <a href="?r=farmer/default/activateFarmers"> <strong>>> aqui <<</strong></a>`
                        );
                    }
                } else {
                    $('#farmerName').removeAttr('disabled');
...
$(document).on("change", "#foodNotice", function () {
    ...
    $.ajax({
        type: 'POST',
        url: "?r=farmer/default/getFoodNoticeItems",
        cache: false,
        data: {
            notice: $notice,
        }
...
$(document).on("click", "#save-farmer", function () {
    ...
    if(name == "" || cpf == "" || phone == "" || groupType == "Selecione o Grupo") {
        ...
        $.ajax({
            type: 'POST',
            url: "?r=farmer/default/updateFarmerRegister",
            cache: false,
            data: {
                farmerId: id,
                name: name,
            ...
            window.location.href = "?r=farmer/default/index";
        });
    } else {
        $.ajax({
            type: 'POST',
            url: "?r=farmer/default/createFarmerRegister",
            cache: false,
            data: {
                name: name,
                cpf: cpf,
