    ...
    table = $("#course-classes").DataTable({
        ...
        columns: [
            ...
            { data: "resources", visible: false },
            {
                ...
                orderable: false,
                defaultContent:
                    '<img class="t-accordion__icon" src="themes/default/img/Glyph.svg" />',
            },
        ],
        language: {
...
function format_validate(d) {
    ...
    let ability = $(
        '<div class="control-group courseplan-ability-container"></div>'
    );
    let abilityLabel = $(
    ...
    let methodology = $(
        '<div class="t-field-text control-group courseplan-methodology-container"></div>'
    );
    ...
    let resourceButtonContainer = $(
        '<div class="t-buttons-container control-group no-margin"></div>'
    );
    let resource = $('<div class="t-field-select control-group"></div>');
    let resourceLabel = $(
        '<label class="t-field-select__label" for="resource">Recurso(s)</label>'
    );
    let resourceInput = $(
        '<div class="t-field-select__input resource-input"></div>'
    );
    let deleteButton = "";
    if (d.deleteButton === "js-unavailable") {
        deleteButton = $(
            '<div class="t-buttons-container"><a class="t-button-danger js-remove-course-class js-unavailable t-button-danger--disabled" data-toggle="tooltip" data-placement="left" title="Aula já ministrada em alguma turma. Não é possível removê-la do plano de aula.">Excluir Plano</a></div>'
        );
    } else {
        deleteButton = $(
            '<div class="t-buttons-container"><a class="t-button-danger js-remove-course-class">Excluir Plano</a></div>'
        );
    }
    ...
    if (d.abilities !== null) {
        let uniqueDisciplines = [
            ...new Set(d.abilities.map((item) => item.discipline)),
        ];
        $.each(uniqueDisciplines, function (i, discipline) {
            ...
            $.each(d.abilities, function (i, v) {
                if (v.discipline == discipline) {
                    div +=
                        '<div class="ability-panel-option"><input type="hidden" class="ability-panel-option-id" value="' +
                        v.id +
                        '" name="course-class[' +
                        d.class +
                        "][ability][" +
                        i +
                        ']"><i class="fa fa-check-square"></i><span>(<b>' +
                        v.code +
                        "</b>) " +
                        v.description +
                        "</span></div>";
    ...
    if (d.resources !== null) {
        $.each(d.resources, function (i, v) {
            ...
            let div = $(
                '<div class="row t-badge-content course-class-resource"></div>'
            );
            let values = $(
...
function format(d) {
    ...
    let abilitySelect = $(
        '<input id="ability-search-select" class="ability-search-select" name="resource" />'
    );
    let abilitiesContainer = $('<div class="courseplan-abilities-selected">');
    ...
    let resourceButton = $(
        '<button class="t-button-primary add-new-resource" style="height: 28px;gap: 5px" ><icon class="t-icon-start"></icon>Adicionar recursos</button>'
    );
    let resource = $('<div class="t-field-select control-group"></div>');
    let resourceLabel = $(
        '<label class="t-field-select__label" for="resource">Recurso(s)</label>'
    ...
    let resourceAdd = $(
        '<button class="add-resource" style="height: 30px;width: 30px;margin-left:10px;" ><icon class="t-icon-plus"></icon></button>'
    );
    let deleteButton = "";
    if (d.deleteButton === "js-unavailable") {
        deleteButton = $(
    ...
    if (resourceInput !== "")
        $.ajax({
            ...
            success: function (response) {
                ...
                // callback(validate);
                if (validate) callback(newRes, newResP);
            },
        });
}
...
function appendNewResource(newResource, newResourceP) {
    ...
    const closeBt =
        '<span class="remove-new-resource"><i class="t-icon-close"></i></span></div>';
...
function addResource(button) {
    ...
    if (
        resources.find(
            ".resource-value[value=" + div.children("select").val() + "]"
        ).length
    ) {
        let resource = resources
            .find(".resource-value[value=" + div.children("select").val() + "]")
            .parent();
        ...
        if (
            ...
            let divRes = $(
                '<div class="t-badge-content course-class-resource"></div>'
            );
            let values = $(
                '<input class="resource-id" type="hidden" name="course-class[' +
                    d.class +
...
function buildAbilityStructureSelect(data) {
    let div =
        '<div class="control-group ability-structure-container"><label>' +
        data.selectTitle +
        '</label><select class="ability-structure-select"><option value="">Selecione...</option>';
...
function buildAbilityStructurePanel(data) {
    ...
    $.each(data.options, function () {
        let selected = $(".js-abilities-selected").find(
            ".ability-panel-option-id[value=" + this.id + "]"
        ).length
            ? "selected"
            : "";
        panel +=
            ...
            selected +
            '"><input type="hidden" class="ability-panel-option-id" value="' +
            this.id +
            '"><i class="fa fa-plus-square"></i><span>(<b>' +
            this.code +
            "</b>) " +
            this.description +
            "</span></div>";
