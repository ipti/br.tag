...
class FoodMenu extends TagModel
{
	...
	public function rules()
	{
		...
		return array(
			array('description, start_date, final_date', 'required'),
			array('include_saturday', 'numerical', 'integerOnly'=>true),
			array('description, observation', 'length', 'max'=>100),
			array('week', 'length', 'max'=>1),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, description, observation, start_date, final_date, week, include_saturday', 'safe', 'on'=>'search'),
		);
	}
