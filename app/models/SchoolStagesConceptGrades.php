 ...
 */
class SchoolStagesConceptGrades extends TagModel
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('school_fk, edcenso_stage_vs_modality_fk', 'required'),
			array('edcenso_stage_vs_modality_fk', 'numerical', 'integerOnly'=>true),
			array('school_fk', 'length', 'max'=>8),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, school_fk, edcenso_stage_vs_modality_fk', 'safe', 'on'=>'search'),
		);
	}
