...
class EdcensoStageVsModality extends TagModel
{
    ...
    {
        Yii::beginProfile('getValidStages', 'application');

        $params = [':year' => Yii::app()->user->year];
        $sql = '';

        if (Yii::app()->getAuthManager()->checkAccess('instructor', Yii::app()->user->loginInfos->id)) {
            $params[':userid'] = Yii::app()->user->loginInfos->id;

            $sql = "SELECT esvm.* FROM edcenso_stage_vs_modality esvm
                ...
                WHERE ii.users_fk = :userid AND school_year = :year
                GROUP BY esvm.id
                ORDER BY esvm.name";
        } else {
            $sql = "SELECT esvm.* FROM edcenso_stage_vs_modality esvm
        ...
        $result = self::model()->findAllBySql($sql, $params);

        Yii::endProfile('getValidStages', 'application');
