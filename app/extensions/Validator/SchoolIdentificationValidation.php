<?php

$DS = DIRECTORY_SEPARATOR;

require_once dirname(__FILE__) . $DS . 'register.php';

//Registro 00
class SchoolIdentificationValidation extends Register
{
    //campo 1
    public function isRegisterType00($register_type)
    {
        if (strlen($register_type) > 2) {
            return ['status' => false, 'erro' => 'As linhas devem ser iniciadas com o número do registro.'];
        }

        if ($register_type != 00) {
            return ['status' => false, 'erro' => 'O registro declarado <tipo de registro> não faz parte do escopo do educacenso.'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 2
    /*@todo
    4. Deve está de acordo com a UF informada no campo 18 (UF).
    5. O código deve ser de uma entidade existente no cadastro do Inep.
    6. A escola informada deve estar entre as escolas abrangidas pelo perfil do informante.
    7. A escola informada não pode ter sido extinta em anos anteriores.
   */

    public function isInepIdValid($inep_id)
    {
        if (empty($inep_id)) {
            return ['status' => false, 'erro' => 'O campo Código de escola - está vazio'];
        }

        if (strlen($inep_id) != 8) {
            return ['status' => false, 'erro' => 'O campo Código de escola - Inep está com tamanho diferente de 8 caracteres'];
        }

        if (!is_numeric($inep_id)) {
            return ['status' => false, 'erro' => 'O campo Código de escola - Inep foi preenchido com um valor não númerico'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 3
    public function isManagerCPFValid($manager_cpf)
    {
        if (empty($manager_cpf)) {
            return ['status' => false, 'erro' => 'O campo Número do CPF do Gestor Escolar não está preenchido'];
        }

        if (strlen($manager_cpf) > 11) {
            return ['status' => false, 'erro' => 'O campo Número do CPF do Gestor Escolar está com tamanho diferente de 11 caracteres'];
        }

        // se nao for numerico
        if (!is_numeric($manager_cpf)) {
            return ['status' => false, 'erro' => 'O campo Número do CPF do Gestor Escolar tem que ser um valor númerico'];
        }

        // se for 0000000000, 1111111
        if (preg_match('/^(.)\1*$/', $manager_cpf)) {
            return ['status' => false, 'erro' => 'O campo Número do CPF do Gestor Escolar não pode ser 0000000000 ou 1111111'];
        }

        if ($manager_cpf == '00000000191') {
            return ['status' => false, 'erro' => 'O campo Número do CPF do Gestor Escolar não pode ser 00000000191'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 4
    public function isManagerNameValid($manager_name)
    {
        if (empty($manager_name)) {
            return ['status' => false, 'erro' => 'O campo Nome do Gestor Escolar não pode ser vazio'];
        }

        if (strlen($manager_name) > 100) {
            return ['status' => false, 'erro' => 'O campo Nome do Gestor Escolar foi preenchido com mais de 100 caracteres'];
        }

        $regex = "/^[A-Z0-9°ºª\- ]/";
        if (!preg_match($regex, $manager_name)) {
            return ['status' => false, 'erro' => 'O campo Nome do Gestor Escolar foi preenchido com caracteres inválidos'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //cargo do gestor campo 5
    public function isManagerRoleValid($manager_role)
    {
        if (empty($manager_role)) {
            return ['status' => false, 'erro' => 'O campo Cargo do Gestor Escolar não pode ser vazio.'];
        }

        if ($manager_role != 1 && $manager_role != 2) {
            return ['status' => false, 'erro' => 'O campo Cargo do Gestor Escolar foi preenchido com um valor diferente de 1 ou 2'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //address eletronico do gestor campo 6
    public function isManagerEmailValid($manager_email)
    {
        if (empty($manager_email)) {
            return ['status' => false, 'erro' => 'O campo Endereço eletrônico (e-mail) do Gestor Escolar não pode ser vazio'];
        }

        if (strlen($manager_email) > 50) {
            return ['status' => false, 'erro' => 'O campo Endereço eletrônico (e-mail) do Gestor Escolar não pode ser maior que 50 caracteres'];
        }

        if (!preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,})$/i", $manager_email)) {
            return ['status' => false, 'erro' => 'O campo Endereço eletrônico (e-mail) do Gestor Escolar  foi preenchido com um valor inválido.'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //situacao de funcionamento campo 7
    public function isSituationValid($situation)
    {
        if (empty($situation)) {
            return ['status' => false, 'erro' => 'O campo Situação de funcionamento não pode ser vazio'];
        }
        if ($situation != 1 && $situation != 2 && $situation != 3) {
            return ['status' => false, 'erro' => 'O campo Situação de funcionamento não pode ser preenchido com valores diferentes de 1,2 ou 3'];
        }
        return ['status' => true, 'erro' => ''];
    }

    //auxiliar dos campos 8 e 9
    public function isDateValid($date)
    {
        if ($date == '' || $date == null) {
            return ['status' => false, 'erro' => 'Data no formato incorreto'];
        }

        $data = explode('/', $date);
        $dia = $data[0];
        $mes = $data[1];
        $ano = $data[2];

        // verifica se a data é valida
        if (!checkdate($mes, $dia, $ano)) {
            return ['status' => false, 'erro' => 'Data no formato incorreto'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 8 e 9
    public function isSchoolYearValid($initial_date, $final_date)
    {
        if (empty($initial_date) || empty($final_date)) {
            return ['status' => false, 'erro' => 'Data de inicio e final do ano letivo não pode ser vazio'];
        }
        $first_result = $this->isDateValid($initial_date);
        $second_result = $this->isDateValid($final_date);
        if (!($first_result['status'] && $second_result['status'])) {
            return ['status' => false, 'erro' => 'Data de inicio e final do ano letivo não está em um formato válido'];
        } else {
            $dataInicial = explode('/', $initial_date);
            $diaInicial = $dataInicial[0];
            $mesInicial = $dataInicial[1];
            $anoInicial = $dataInicial[2];

            $dataFinal = explode('/', $final_date);
            $diaFinal = $dataFinal[0];
            $mesFinal = $dataFinal[1];
            $anoFinal = $dataFinal[2];

            // se a data inicial do periodo letivo é menor que a data final
            if ($anoInicial < $anoFinal) {
                return ['status' => true, 'erro' => ''];
            } elseif ($anoInicial == $anoFinal) {
                if ($mesInicial < $mesFinal) {
                    return ['status' => true, 'erro' => ''];
                }
                if ($mesInicial == $mesFinal) {
                    if ($diaInicial < $diaFinal) {
                        return ['status' => true, 'erro' => ''];
                    }
                    if ($diaInicial >= $diaFinal) {
                        return ['status' => false, 'erro' => 'Dia inicial é maior ou igual a Dia Final'];
                    }
                }
                if ($mesInicial > $mesFinal) {
                    return ['status' => false, 'erro' => 'Mes Inicial está maior que Mes Final'];
                }
            } else {
                return ['status' => false, 'erro' => 'Ano letivo inicial está maior que o ano final'];
            }
        }
    }

    //campo 10
    public function isSchoolNameValid($name)
    {
        //deve ser no minimo 4
        if (strlen($name) == 0) {
            return ['status' => false, 'erro' => 'O campo Nome da escola não pode ser vazio'];
        }
        if (strlen($name) < 4) {
            return ['status' => false, 'erro' => 'O campo Nome da escola não pode ter menos de 4 caracteres'];
        }
        if (strlen($name) > 100) {
            return ['status' => false, 'erro' => 'O campo Nome da escola tem mais de 100 caracteres'];
        }
        if (!(preg_match('/^[a-z\d°ºª\- ]{4,20}/i', $name))) {
            return ['status' => false, 'erro' => 'O campo Nome da escola possui caracteres inválidos'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 11
    public function isLatitudeValid($latitude)
    {
        if (strlen($latitude) > 20) {
            return ['status' => false, 'erro' => 'O campo Latitude não pode ter mais de 20 caracteres'];
        }
        $regex = '/^[0-9.-]+$/';
        if (!preg_match($regex, $latitude)) {
            return ['status' => false, 'erro' => 'O campo Latitude contém caractere(s) inválido(s).'];
        }
        if (!($latitude >= -33.750833 && $latitude <= 5.272222)) {
            return ['status' => false, 'erro' => 'O campo Latitude foi preenchido com um valor que não está entre -33.750833 e 5.272222'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 12
    public function isLongitudeValid($longitude)
    {
        if (strlen($longitude) > 20) {
            return ['status' => false, 'erro' => 'O campo Longitude não pode ter mais de 20 caracteres'];
        }
        $regex = '/^[0-9.-]+$/';
        if (!preg_match($regex, $longitude)) {
            return ['status' => false, 'erro' => 'O campo Longitude contém caractere(s) inválido(s).'];
        }
        if (!($longitude >= -73.992222 && $longitude <= -32.411280)) {
            return ['status' => false, 'erro' => 'O campo Longitude foi preenchido com um valor que não está entre -73.99222 e -32.411280'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 13
    public function isCEPValid($cep)
    {
        if (empty($cep)) {
            return ['status' => false, 'erro' => 'O campo CEP não pode ser vazio'];
        }
        if (strlen($cep) != 8) {
            return ['status' => false, 'erro' => 'O campo CEP deve ter 8 caracteres'];
        }
        if (!is_numeric($cep)) {
            return ['status' => false, 'erro' => 'O campo CEP deve ser preenchido com um valor numerico'];
        }
        if (preg_match('/^(.)\1*$/', $cep)) {
            return ['status' => false, 'erro' => 'O campo CEP não pode ser a repetição de um único algarismo'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 14,campo 15,campo 16,campo 17,campo 18,campo 19,campo 20
    public function isAddressValid($address, $allowed_lenght, $optional)
    {
        $regex = '/^[0-9 a-z.,-ºª ]/';

        if (!$optional && $address === '') {
            return ['status' => false, 'erro' => 'O campo endereço é obrigatório.'];
        }

        if (strlen($address) > $allowed_lenght) {
            return ['status' => false, 'erro' => "O campo de endereço - $address - não pode ter mais de $allowed_lenght caracteres."];
        }
        if (!preg_match($regex, $address)) {
            return ['status' => false, 'erro' => "O campo de endereço - $address - foi preenchido com caracteres inválidos"];
        }

        return ['status' => true, 'erro' => ''];
    }

    public function checkPhoneNumbers($ddd, $phoneNumber, $otherPhoneNumber)
    {
        if ($ddd !== '' && $phoneNumber === '' && $otherPhoneNumber === '') {
            return ['status' => false, 'erro' => 'Quando o DDD é prenchido, pelo menos um dos telefones também deve ser preenchido.'];
        } elseif ($ddd === '' && ($phoneNumber !== '' || $otherPhoneNumber !== '')) {
            return ['status' => false, 'erro' => 'Quando um dos telefones é preenchido, o DDD também deve ser preenchido.'];
        } elseif ($ddd !== '' && strlen($ddd) != 2) {
            return ['status' => false, 'erro' => 'DDD deve conter 2 caracteres.'];
        } elseif ($phoneNumber !== '' && $otherPhoneNumber !== '' && ($phoneNumber === $otherPhoneNumber)) {
            return ['status' => false, 'erro' => 'Os dois campos de números de telefone não podem ser iguais.'];
        } elseif (($phoneNumber !== '' && (strlen($phoneNumber) < 8 || strlen($phoneNumber) > 9))
            || ($otherPhoneNumber !== '' && (strlen($otherPhoneNumber) < 8 || strlen($otherPhoneNumber) > 9))) {
            return ['status' => false, 'erro' => 'Os campos de telefone, quando preenchidos, devem conter 8 ou 9 caracteres.'];
        } elseif (($phoneNumber !== '' && !is_numeric($phoneNumber)) || ($otherPhoneNumber !== '' && !is_numeric($otherPhoneNumber))) {
            return ['status' => false, 'erro' => 'Apenas números podem ser informados.'];
        } elseif (($phoneNumber !== '' && strlen($phoneNumber) === 9 && substr($phoneNumber, 0, 1) !== '9')
            || ($otherPhoneNumber !== '' && strlen($otherPhoneNumber) === 9 && substr($otherPhoneNumber, 0, 1) !== '9')) {
            return ['status' => false, 'erro' => 'Quando o telefone tiver 9 caracteres, o primeiro caractere deve ser o dígito 9.'];
        } elseif (($phoneNumber !== '' && count(array_unique(str_split($phoneNumber)))) === 1 || ($otherPhoneNumber !== '' && count(array_unique(str_split($otherPhoneNumber))))) {
            return ['status' => false, 'erro' => 'Os campos de telefone não podem ser a repetição de um mesmo algarismo.'];
        }
        return ['status' => true, 'erro' => ''];
    }

    //campo 26
    public function isEmailValid($email)
    {
        if (strlen($email) > 50) {
            return ['status' => false, 'erro' => 'Email com tamanho invalido'];
        }

        if (!preg_match(
            "/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,})$/i",
            $email
        )) {
            return ['status' => false, 'erro' => 'Email com formato invalido'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 28
    public function isAdministrativeDependenceValid($value, $uf)
    {
        $result = $this->isAllowed($value, ['1', '2', '3', '4']);
        if (!$result['status']) {
            return ['status' => false, 'erro' => $result['erro']];
        }

        if ($uf == '53') {
            if ($value == '3') {
                return ['status' => false, 'erro' => 'Dependencia Administrativa inválida'];
            }
        }
        return ['status' => true, 'erro' => ''];
    }

    //campo 29
    public function isLocationValid($value)
    {
        if ($value != 1 && $value != 2) {
            return ['status' => false, 'erro' => 'Lozalização inválida'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //auxiliar nos campos 30,31,32
    public function isField7And28Valid($inep_id, $schoolSituation, $dependency)
    {
        //campo 7 deve ser igual a 1.. Campo 28 deve ser igual a 4
        if ($schoolSituation == 1 && isSituationValid($schoolSituation) == true
            && isAdministrativeDependenceValid($inep_id, $dependency) == true && $dependency == 4) {
            return true;
        } else {
            return false;
        }
    }

    //campo 30
    public function checkPrivateSchoolCategory($value, $situation, $administrative_dependence)
    {
        if ($situation == '1' && $administrative_dependence == '4') {
            $result = $this->isAllowed($value, ['1', '2', '3', '4']);
            if (!$result['status']) {
                return ['status' => false, 'erro' => $result['erro']];
            }
        } else {
            if ($value != null) {
                return ['status' => false, 'erro' => "Valor $value deveria ser nulo"];
            }
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 31

    public function isPublicContractValid($value, $situation, $administrative_dependence)
    {
        if (!($situation == '1' && $administrative_dependence == '4')) {
            if ($value != null) {
                return ['status' => false, 'erro' => "Valor $value deveria ser nulo"];
            } else {
                return ['status' => true, 'erro' => ''];
            }
        } else {
            $result = $this->isAllowed($value, ['1', '2', '3']);
            if (!$result['status']) {
                return ['status' => false, 'erro' => $result['erro']];
            }
        }
        return ['status' => true, 'erro' => ''];
    }

    //campos 32 a 36

    public function isPrivateSchoolMaintainerValid($keepers, $situation, $administrative_dependence)
    {
        if ($situation == '1' && $administrative_dependence == '4') {
            $result = $this->atLeastOne($keepers);
            if (!$result['status']) {
                return ['status' => false, 'erro' => $result['erro']];
            }
        } else {
            foreach ($keepers as $key => $value) {
                if ($value != null) {
                    return ['status' => false, 'erro' => 'Valor deveria ser nulo'];
                }
            }
        }

        return ['status' => true, 'erro' => ''];
    }

    //para os campos 37 e 38
    public function isCNPJValid($cnpj, $situation, $administrative_dependence)
    {
        if (!($situation == '1' && $administrative_dependence == '4')) {
            if ($cnpj != null) {
                return ['status' => false, 'erro' => "Valor $value deveria ser nulo"];
            } else {
                return ['status' => true, 'erro' => ''];
            }
        }

        if (!is_numeric($cnpj)) {
            return ['status' => false, 'erro' => 'CNPJ está com padrão inválido'];
        }
        if (strlen($cnpj) != 14) {
            return ['status' => false, 'erro' => 'O CNPJ está com tamanho errado'];
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 39
    public function isRegulationValid($value, $schoolSituation)
    {
        //campo 7 deve ser igual a 1
        if ($schoolSituation == 1) {
            if ($value != 0 && $value != 1 && $value != 2) {
                return ['status' => false, 'erro' => 'Regulamentação da escola errada'];
            }
        } else {
            if ($value != null) {
                return ['status' => false, 'erro' => "Valor $value deveria ser nulo"];
            }
        }

        return ['status' => true, 'erro' => ''];
    }

    //campo 40,41 e 42
    public function isOfferOrLinkedUnity(
        $value,
        $InepCode,
        $HeadSchool,
        $schoolSituation,
        $hostedcenso_city_fk,
        $atualedcenso_city_fk,
        $hostDependencyAdm,
        $atualDependencyAdm,
        $IESCode
    ) {
        if ($value == 1) {
            return isInepHeadSchoolValid($InepCode, $HeadSchool, $schoolSituation, $hostedcenso_city_fk, $atualedcenso_city_fk, $hostDependencyAdm, $atualDependencyAdm);
        }
        if ($value == 2) {
            return isIESCodeValid($IESCode, $schoolSituation);
        }
    }

    //41
    public function inepHeadSchool(
        $value,
        $offer_or_linked_unity,
        $current_inep_id,
        $head_school_situation,
        $head_of_head_school
    ) {
        if ($offer_or_linked_unity == '1') {
            if ($value == '' && $value == null) {
                return ['status' => false, 'erro' => 'O campo não foi preenchido quando deveria ser preenchido.'];
            }
            if (!is_numeric($value)) {
                return ['status' => false, 'erro' => 'Apenas números são permitidos'];
            }
            if (strlen($value) != 8) {
                return ['status' => false, 'erro' => 'Deve conter 8 caracteres'];
            }
            if ($value == $current_inep_id) {
                return ['status' => false, 'erro' => 'Não pode ser preenchido com o mesmo código INEP da escola.'];
            }
            if ($head_school_situation != '1') {
                return ['status' => false, 'erro' => 'Escola sede não está em atividade'];
            }
            if (substr($current_inep_id, 0, 2) != substr($value, 0, 2)) {
                return ['status' => false, 'erro' => 'Escolas não são do mesmo estado'];
            }
            if ($current_inep_id == $head_of_head_school) {
                return ['status' => false, 'erro' => 'Escola sede não pode ter atual como sede'];
            }
        } else {
            if ($value != null) {
                return ['status' => false, 'erro' => "Valor $value deveria ser nulo"];
            }
        }

        return ['status' => true, 'erro' => ''];
    }

    //42
    public function iesCode($value, $administrativeDependence, $offer_or_linked_unity)
    {
        if ($offer_or_linked_unity == '2') {
            if ($value == '' && $value == null) {
                return ['status' => false, 'erro' => 'O campo não foi preenchido quando deveria ser preenchido.'];
            }
            if (!is_numeric($value)) {
                return ['status' => false, 'erro' => 'Apenas números são permitidos'];
            }
            $institutionType = 'PRIVADA';
            if ($administrativeDependence == '1' || $administrativeDependence == '2' || $administrativeDependence == '3') {
                $institutionType = 'PUBLICA';
            }
            $sql = "SELECT 	COUNT(id) AS status
			FROM 	edcenso_ies
			WHERE 	id = ' . $value . ' AND working_status = 'ATIVA'
					AND institution_type = '$institutionType';";
            $check = Yii::app()->db->createCommand($sql)->queryAll();
            if ($check[0]['status'] != '1') {
                return ['status' => false, 'erro' => 'O código IES deve ser preenchido com um valor permitido, estar ativo e ser da mesma dependência administrativa.'];
            }
        } else {
            if ($value != null || $value != '') {
                return ['status' => false, 'erro' => "Valor $value deveria ser nulo"];
            }
        }

        return ['status' => true, 'erro' => ''];
    }

    public function isNotNullValid($value)
    {
        if ($value == null) {
            return ['status' => false, 'erro' => 'O campo não foi preenchido quando deveria ser preenchido'];
        }
        return ['status' => true, 'erro' => ''];
    }

    public function isValidLinkedOrgan($administrativeDependence, $linkedMec, $linkedArmy, $linkedHealth, $linkedOther)
    {
        if ($administrativeDependence == '1' || $administrativeDependence == '2' || $administrativeDependence == '3') {
            if (($linkedMec == null || $linkedMec == '0') && ($linkedArmy == null || $linkedArmy == '0')
                && ($linkedHealth == null || $linkedHealth == '0') && ($linkedOther == null || $linkedOther == '0')) {
                return ['status' => false, 'erro' => 'Quando a dependência administrativa for federal, estadual ou municipal, o campo se torna obrigatório.'];
            }
        } else {
            if ($linkedMec != null || $linkedArmy != null || $linkedHealth != null || $linkedOther != null) {
                return ['status' => false, 'erro' => 'Quando a dependência administrativa for privada, as opções do campo deve ser todas vazias.'];
            }
        }
        return ['status' => true, 'erro' => ''];
    }

    public function regulationOrganSphere($administrativeDependence, $federal, $state, $municipal)
    {
        if (($federal == 0 || $federal == null) && ($state == 0 || $state == null) && ($municipal == 0 || $municipal == null)) {
            return ['status' => false, 'erro' => 'Selecione pelo menos uma opção.'];
        }

        if ($federal == 1 && ($administrativeDependence == 2 || $administrativeDependence == 3)) {
            return ['status' => false, 'erro' => "Quando a dependência administrativa for Estadual ou Municipal, não pode selecionar 'Órgão Federal'."];
        }
        if ($municipal == 1 && ($administrativeDependence == 1 || $administrativeDependence == 2)) {
            return ['status' => false, 'erro' => "Quando a dependência administrativa for Federal ou Estadual, não pode selecionar 'Órgão Municipal'."];
        }

        if ($municipal == 1 && $federal == 1) {
            return ['status' => false, 'erro' => "Os campos 'Órgão Municipal' e 'Órgão Federal' não podem ser ambos selecionados."];
        }
        return ['status' => true, 'erro' => ''];
    }
}
