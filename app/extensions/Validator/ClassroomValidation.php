...
class ClassroomValidation extends Register
{
    ...
    public function isValidAssistanceType($school_structure, $assistance_type, $mediation)
    {
        ...
        for ($i = 0; $i < count($operation_locations); ++$i) {
    ...
    public function hasDecriptedComplementaryActivity($activities)
    {
        if (empty($activities) || !is_array($activities)) {
            return false;
        }

        $count = EdcensoComplementaryActivityType::model()->countByAttributes(
            ['deprecated' => 1],
            'id IN ('.implode(',', array_map('intval', $activities)).')'
        );
    ...
    public function isValidComplementaryActivityType($activities, $complementaryActivity)
    {
        ...
        if (1 == $complementaryActivity) {
            ...
            $hasDecriptedCAct = $this->hasDecriptedComplementaryActivity($activities);
            if ($hasDecriptedCAct) {
                return ['status' => false, 'erro' => 'Desmarque a(s) atividade(s) complementar inválida'];
            }
        } else {
