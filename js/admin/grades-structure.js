...
class UnitiesPanel extends HTMLElement {
    constructor() {
        super();
    }
    ...
    connectedCallback() {
        ...
        const self = this;
        this.fetchUnities().then(function (response) {
            const unities = response.unities.map((e, index) => {
                return {
                    index: index,
                    name: e.name,
                    type: e.type,
            ...
            self.setAttribute("unities", JSON.stringify(unities));
            shadow.innerHTML = "";
            shadow.appendChild(self.styles());
            shadow.appendChild(self.build());
        });
    }

    updateUnities(unity) {
        const unities = this.hasAttribute("unities")
            ? JSON.parse(this.getAttribute("unities"))
            : [];

        const updatedUnities = unities.map((e) => {
            if (e.index == unity.index) {
                return { ...unity };
            }
            return e;
        });

        this.setAttribute("unities", JSON.stringify(updatedUnities));
    }

    attributeChangedCallback(property, oldValue, newValue) {
        if (oldValue === newValue) return;
        this[property] = newValue;
        window.EventBus.fire("panelChanged", this.attributes);
    }

    async fetchUnities() {
        const response = await fetch("?r=admin/getUnities", {
            method: "POST",
            cache: "no-store",
            headers: {
    ...
    build() {
        ...
        const unities = this.hasAttribute("unities")
            ? JSON.parse(this.getAttribute("unities"))
            : [];
        const unitiesElements = unities.map((e, index) => {
            const unityComponent = new UnityComponent();
            unityComponent.setAttribute("index", index);
            unityComponent.name = e.name;
            unityComponent.setAttribute("type", e.type);
            unityComponent.setAttribute("formula", e.formula);
            unityComponent.setAttribute(
                "modalities",
                JSON.stringify(e.modalities)
            );

            console.log(unityComponent.attributes)
            unityComponent.addEventListener("unityChanged", (event) => {
                self.updateUnities(event.detail);
            });

            return unityComponent;
        });
        const componentRoot = document.createElement("div");
        unitiesElements.forEach((e) => componentRoot.appendChild(e));
...
class UnityComponent extends HTMLElement {
    static get observedAttributes() {
        return ["index", "name", "type", "formula", "modalities"];
    }
    constructor() {
        super();
        this._name = null;
    ...
    set name(value) {
        // debugger;
        if (value !== this.name) {
            this._name = value; // Set the internal value
            $(this).find(".unity-name").val(value);
        }
    }

    connectedCallback() {
        const self = this;
        window.EventBus.register("modalityChanged", (evt) => {
            const newModality = evt.detail;
            const modalities = self.hasAttribute("modalities")
                ? JSON.parse(self.getAttribute("modalities"))
                : [];

            const updatedModalities = modalities.map((e, index) => {
                if (e.index == newModality.index) {
                    return { ...newModality };
                }
                return { index: index, ...e };
            });

            self.setAttribute("modalities", JSON.stringify(updatedModalities));
        });
    ...
    attributeChangedCallback(property, oldValue, newValue) {
        ...
        if (property === "name") {
            this.shadow.querySelector(".unity-title").innerHTML = newValue;
        }
    }

    addModality() {
        const hasModalities = this.hasAttribute("modalities");
        const modalitiesAttr = hasModalities
            ? JSON.parse(this.getAttribute("modalities"))
            : [];
        modalitiesAttr.push({ name: "" });
        this.setAttribute("modalities", JSON.stringify(modalitiesAttr));
        this.shadow.querySelector(".js-modality-container").innerHTML =
            this._buildModalities(modalitiesAttr);
    }
    ...
    build() {
        ...
        const indexAttr = this.getAttribute("index");
        const nameAttr = this.getAttribute("name");
        const formulaAttr = this.getAttribute("formula");
        const modalitiesAttr = JSON.parse(this.getAttribute("modalities"));

        const isUnityConcept = typeAttr === "UC";
        const isCollapsed = indexAttr == 0 ? "in" : "";

        const typeItems = this._buildTypeItems(isUnityConcept, typeAttr);
        const formulasItems = this._buildFormulasItems(formulaAttr);

        const unityHtml = template`
        <div class='unity column is-three-fifths'>
            <div class='row unity-heading ui-accordion-header'>
                <a class="accordion-toggle" data-toggle="collapse" data-parent="#accordion" href="#collaps-${indexAttr}">
                    <h2 class="unity-title accordion-heading">${
                        nameAttr ?? "Unidade:"
                    } </h2>
                </a>
                <span class="remove-button js-remove-unity t-button-icon-danger t-icon-trash  js-change-cursor"></span>
            </div>
            <div id="collaps-${indexAttr}"class=" collapse ${isCollapsed}">
                <input type='hidden' class="unity-id">
                <input type="hidden" class="unity-operation" value="create">
                <div class="t-field-text" style="margin-top: 16px">
                    <label class='t-field-text__label--required'>Nome: </label>
                    <input type='text' class='t-field-text__input unity-name' value="${nameAttr}" placeholder='1ª Unidade, 2ª Unidade, Recuperação Final, etc.'>
                </div>
                <div class="t-field-select">
                    <label class='t-field-select__label--required'>Modelo: </label>
                    <select class='t-field-select__input js-type-select select-search-on control-input'>
                ...
                <div name="modalities" class="t-cards js-modality-container">
                    ${this._buildModalities(modalitiesAttr)}
                </div>
            </div>
        </div>`;
        ...
        const buttonAdd = componentRoot.querySelector(".js-new-modality");
        buttonAdd.addEventListener("click", (e) => this.addModality());

        const nameInput = componentRoot.querySelector(".unity-name");
    ...
    _buildTypeItems(isUnityConcept, selectedType) {
        ...
        const optionBuilder = (e) => {
            const isSelected = selectedType === e.key ? "selected" : "";
    ...
    _buildModalities(modalitiesAttr) {
        if (this.hasAttribute("modalities")) {
            const modalitiesElements = modalitiesAttr.map((e, index) => {
                return template`<unity-modality-component index=${index} name="${e.name}"></unity-modality-component>`;
            });

            return modalitiesElements.join("");
        }
        return "";
    }
}
...
class UnityModalityComponent extends HTMLElement {
    ...
    attributeChangedCallback(property, oldValue, newValue) {
        if (oldValue === newValue) return;
        this[property] = newValue;
        window.EventBus.fire("modalityChanged", {
            index: this.index,
            name: this.name,
        });
    }
    ...
    build() {
        ...
        const hasWeight = this.hasAttribute("weight");

        const weightTemplate = template`
            <div class="t-field-text">
                <label class='t-field-text__label--required'>Peso:</span></label>
                <input type='text' class='t-field-text__input weight form-control' value="${weightAttr}" placeholder='Peso'>
            </div>
        `;
            ...
            <div class='modality' concept='0'>
                ...
                <div class="row">
                    <div class="t-field-text">
                        ...
                        <input type='text' class='modality-name t-field-text__input' modalitytype='C' value="${nameAttr}" placeholder='Prova, Avaliação, Trabalho, etc.' style='width: calc(100% - 222px);'>
                        ${hasWeight ? weightTemplate : ""}
                    </div>
        ...
        const nameInput = componentRoot.querySelector(".modality-name");
        nameInput.addEventListener("input", (e) =>
            this.setAttribute("name", e.target.value)
        );
...
window.EventBus.register("unityChanged", (evt) => console.log(evt));
window.EventBus.register("modalityChanged", (evt) => console.log(evt));
window.EventBus.register("panelChanged", (evt) => console.log(evt));

customElements.define("unities-panel", UnitiesPanel);
