    ...
    success: function (response) {
      if (response) {
        if (response == 1) {
          ...
          if (obj.hasClass("red")) {
            let newArr = store.report[obj.data("student")].faults.map(function (
              item
            ) {
              if (
                item.day != obj.data("day") &&
                item.schedule != obj.data("schedule")
              ) {
                return item;
              }
            });
            store.report[obj.data("student")].faults = newArr;
            obj.removeClass("red");
          } else {
