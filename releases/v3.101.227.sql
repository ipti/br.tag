...
-- app/migrations/2025-13-02_new_view_studentFile/sql.sql
...
-- app/migrations/2025-14-03_classroom_esvm_fieldsize/sql.sql
ALTER TABLE `classroom` MODIFY COLUMN `edcenso_stage_vs_modality_fk` VARCHAR(3) NOT NULL;
...
select max(ic.id)from instance_config ic group by ic.parameter_key having count(1) > 1;

delete from instance_config where id in (select id from temp_configs);

ALTER TABLE instance_config ADD CONSTRAINT instance_config_un UNIQUE KEY (parameter_key);
