...
import { previewHousing, deleteHousing, getHousing } from 'Actions';
...
class Card extends Component{
    ...
    deleteHousing = async (id) => {
        this.props.deleteHousing(id, () => {
            this.props.getHousing();
        });
    }
                        ...
                        this.state.showActions ?(
                            <div className="row mx-0 w-100" onClick = {this.handleClose}>
                                <div className="col-12 d-flex justify-content-center aling-items-center">
                                    <Button className="mr-2" color="primary" onClick={ () => this.props.previewHousing(this.props.id)} >Visualizar</Button>
                                    <Button color="danger" onClick={ () => this.deleteHousing(this.props.id)} >Excluir</Button>
                                </div>
                            </div>
                        ): (
                            ...
                            <div className="row mx-0" onClick = {this.handleClick}>
                                <div className="col-7 d-flex aling-items-center">
                                    ...
                                    <div className="w-100">
                                        <h4 className="housing-card-title text-ellipsis">{child}</h4>
                                        <span className="housing-card-subtitle">Nome</span>
                                    </div>
                                </div>
                                <div className="col-4 d-flex aling-items-center">
                                    ...
                                    <div>
                                        <h4 className="housing-card-title">{createdAt.split(' ')[0]}</h4>
                                        <span className="housing-card-subtitle">Criada em</span>
                                    </div>
                                </div>
                            </div>
                            )
