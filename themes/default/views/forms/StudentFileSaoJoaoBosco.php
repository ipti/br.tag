...
<div class="main-container">
    ...
    <div class="body">
        <div class="enrollment-data">
            ...
            <div>Endereço: <span class="bold"><?php echo $add_data['address']; if ($add_data['number'] != null || $add_data['number'] != '') {
    echo ', ' . $add_data['number'];
} if ($add_data['neighborhood'] != null || $add_data['neighborhood'] != '') {
    echo ', ' . $add_data['neighborhood'];
} ?></span></div>
            <div>Cidade: <span class="bold"><?php echo $city['name']?></span> Estado: <span class="bold"><?php echo strtoupper($state['name']); ?></span><span> CEP: <span class="bold"><?php if ($add_data['cep'] != null || $add_data['cep'] != '') {
    echo $add_data['cep'];
} else {
    echo '_____________________';
} ?></span></span></div>
            <div>Escola de Origem: ___________________ <span>Cidade: ____________ Estado: _____________ </span></div>
            <div style="clear:both;">RG: <?php if ($add_data['rg_number'] != null || $add_data['rg_number'] != '') {
    echo '<span class="bold">' . $add_data['rg_number'] . '</span>';
} else {
    echo '_________ ' ;
}?> Org.Exp.: ________ <span>Data Exp.: <?php if ($add_data['rg_number_expediction_date'] != null || $add_data['rg_number_expediction_date'] != '') {
    echo '<span class="bold">' . $add_data['rg_number_expediction_date'] . '</span>' ;
} else {
    echo '________ ' ;
}?> Data de Nascimento: <span class="bold"><?php echo $student->studentFk->birthday?></span></span></div>
            <div style="clear:both;">Certidão de Nascimento:____________ Email: ________________ <span>Estado Civil: ________</span></div>
            <div>CPF: <?php if ($add_data['cpf'] != null || $add_data['cpf'] != '') {
    echo '<span class="bold">' . $add_data['cpf'] . '</span>';
} else {
    echo '___________ ' ;
}?> Naturalidade: <span class="bold"><?php echo strtoupper($student->studentFk->edcensoUfFk->name)?></span> <span>Nacionalidade: <span class="bold"><?php echo $student->studentFk->edcensoNationFk->name?></span></span></div>
        </div>
        ...
        <div class="responsable-data">
            ...
            <div>Nome do Responsável: <?php if ($student->studentFk->responsable_name != null || $student->studentFk->responsable_name != '') {
    echo '<span class="bold">' . $student->studentFk->responsable_name . '</span>';
} else {
    echo '______________________________________________________ ' ;
}?></div>
            <div>Endereço: _________________________________________________________________</div>
            <div>Cidade: _____________ Estado: _____________ CEP: ________ Telefone: <?php if ($student->studentFk->responsable_telephone != null || $student->studentFk->responsable_telephone != '') {
    echo '<span class="bold">' . $student->studentFk->responsable_telephone . '</span>';
} else {
    echo '____________' ;
}?></div>
            <div>Data de Nascimento: __/__/__ Nacionalidade: _____________ Instrução: ______________ </div>
            <div>CPF: <?php if ($student->studentFk->responsable_cpf != null || $student->studentFk->responsable_cpf != '') {
    echo '<span class="bold">' . $student->studentFk->responsable_cpf . '</span>';
} else {
    echo '______________ ' ;
}?> Celular: _________________ Email: ________________________</div>
            <div>Identidade: <?php if ($student->studentFk->responsable_rg != null || $student->studentFk->responsable_rg != '') {
    echo '<span class="bold">' . $student->studentFk->responsable_rg . '</span>';
} else {
    echo '_____________ ' ;
}?> Profissão:<?php if ($student->studentFk->responsable_job != null || $student->studentFk->responsable_job != '') {
    echo '<span class="bold">' . $student->studentFk->responsable_job . '</span>';
} else {
    echo '______________ ' ;
}?> Telefone Trabalho: _____________</div>
        </div>
    </div>
</div>
<br/>
