    ...
    <div>
        <table class="table table-bordered table-striped" style="font-size: 11px">
            ...
            foreach ($report as $key => $r) {
                //validacoes
                if ($r['received_cc'] == 0) {
                    $received_cc = '<i class="fa fa-close" style="color: black"></i>';
                } else {
                    $received_cc = '';
                };
                if ($r['received_address'] == 0) {
                    $received_address = '<i class="fa fa-close" style="color: black"></i>';
                } else {
                    $received_address = '';
                };
                if ($r['received_photo'] == 0) {
                    $received_photo = '<i class="fa fa-close" style="color: black"></i>';
                } else {
                    $received_photo = '';
                };
                if ($r['received_nis'] == 0) {
                    $received_nis = '<i class="fa fa-close" style="color: black"></i>';
                } else {
                    $received_nis = '';
                };
                if ($r['received_responsable_rg'] == 0) {
                    $received_responsable_rg = '<i class="fa fa-close" style="color: black"></i>';
                } else {
                    $received_responsable_rg = '';
                };
                if ($r['received_responsable_cpf'] == 0) {
                    $received_responsable_cpf = '<i class="fa fa-close" style="color: black"></i>';
                } else {
