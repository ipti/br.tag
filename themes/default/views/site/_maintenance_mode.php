<?php
/* @var $this SiteController */
/* @var $model LoginForm */
/* @var $form CActiveForm  */
?>

<body class="login" style="overflow-x: hidden;">
    <div class="colorful-bar">
        <span id="span-color-blue"></span>
        <span id="span-color-red"></span>
        <span id="span-color-green"></span>
        <span id="span-color-yellow"></span>
    </div>
    <!-- Wrapper -->
    <div id="login">
        <img alt="logo negativa" src="<?php echo Yii::app()->theme->baseUrl; ?>/img/fadedlogo.svg" class="fadedlogo" />
        <img alt="tag" src="<?php echo Yii::app()->theme->baseUrl; ?>/img/tag-title.png"
            style="padding: 20px 20px;position: absolute;top: 0;left: 0;" />
        <div class="login-form">
            <h1>Caro usuário, o TAG se encontra em manutenção</h1>
            <h4 style="text-align:center">Estamos melhorando nossos servidores para melhor atende-los</h4>
            <h2 style="text-align:center">Retorno estimado dia 31/12 2:30 AM, Feliz ano novo!</h2>
        </div>

    </div>
</body>